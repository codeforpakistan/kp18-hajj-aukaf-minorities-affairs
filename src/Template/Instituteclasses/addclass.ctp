<script>
    $(function () {
        $('#minority_students').change(function () {
//    needy_students
            ...
            if ($.isNumeric($(this).val())) {
                $.ajax({
                    type: "GET",
                    contentType: 'json',
                    url: "<?= $this->Url->build(['controller' => 'Instituteclasses', 'action' => 'services']) ?>",
                    data: "fund_id=" + url[6],
                    success: function (data) {
                        var needy_students = ($('#minority_students').val() * data) / 100;
//                        alert(needy_students);
                        $('#needy_students').val(Math.floor(needy_students));
                ...
                $(this).val('');
                $('<p style="color:red">Insert only integer value</p>').insertAfter($('#minority_students'));
            }
        });
    });
</script>
    ...
    <div class="container">
        ...
        <div class="crumbs">
            <ul id="breadcrumbs" class="breadcrumb">
                ...
                <li class="current">
                    <a href="<?= $this->request->webroot . 'Instituteclasses/index/' . $this->request->params['pass'][0]; ?>" title="">List of classes</a>
                </li>
                <li class="current">
                    <a href="#" title="">Add class</a>
                </li>
            </ul>
                        ...
                        <div class="widget-content">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <?= $this->Form->button(__('Click to add'), ['class' => 'btn btn-primary pull-right']); ?>
                                </div>
                            </div> 
                            <!-- /.row -->
                        </div> <!-- /.widget-content -->
